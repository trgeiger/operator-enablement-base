...
RUN curl -Lo ./operator-sdk "https://github.com/operator-framework/operator-sdk/releases/download/v1.25.4/operator-sdk_linux_amd64" && chmod +x ./operator-sdk && mv ./operator-sdk /usr/bin/operator-sdk
...
RUN rpm-ostree install vim zsh distrobox code && \
